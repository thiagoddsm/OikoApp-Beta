...
function IntegrationLaboratory({ isConnected, lastError }: { isConnected: boolean, lastError?: string }) {
    ...
    const handleReadTest = async () => {
        ...
        addLog("Iniciando teste de leitura (Contas Bancárias)...");
    ...
    const handleWriteTest = async () => {
        ...
        addLog("Iniciando teste de escrita (Faturamento v2)...");

        try {
            const res = await fetch('/api/finance/conta-azul/test-write', { method: 'POST' });
            const data = await res.json();
            if (res.ok) {
                addLog("Sucesso! Registro de teste criado via API v2.");
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                        ...
                        <div className="text-left">
                            ...
                            <p className="text-[9px] text-muted-foreground font-normal">Puxar bancos (v1)</p>
                        </div>
                    </Button>
                    ...
                    >
                        ...
                        <div className="text-left">
                            ...
                            <p className="text-[9px] text-muted-foreground font-normal">Gerar fatura teste (v2)</p>
                        </div>
                    </Button>
                </div>
...
function ContaAzulConnect() {
  ...
  const handleConnect = () => {
   ...
   const handleDisconnect = () => {
    if (!configDocRef) return;
    if (window.confirm("Tem certeza que deseja encerrar a conexão? Isso limpará os tokens atuais.")) {
    ...
    <div className="space-y-6">
        <Alert className="bg-blue-50 border-blue-200">
            ...
            <AlertDescription className="text-blue-700 text-xs space-y-2 mt-2">
                <p>No seu Portal Conta Azul, o campo <strong>URL de Redirecionamento</strong> deve ser:</p>
                ...
                <Card className="shadow-lg border-2">
                    <CardHeader className="bg-muted/30 border-b">
                        <div className="flex items-center justify-between">
                            ...
                            {isConnected ? <Badge className="bg-emerald-500">CONECTADO</Badge> : <Badge variant="outline">AGUARDANDO</Badge>}
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-6 pt-6">
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input value={clientId} onChange={(e) => setClientId(e.target.value)} placeholder="881cq0o..." className="font-mono" />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2 border-t pt-4">
                                ...
                                <div className="relative">
                                    <Input 
                                        ...
                                        placeholder="Cole um token gerado manualmente aqui..." 
                                        className="font-mono pr-10 text-xs"
                                    />
                                    <Button variant="ghost" size="icon" className="absolute right-0 top-0 h-10 w-10" onClick={() => setShowToken(!showToken)}>
                                        {showToken ? <EyeOff size={16} /> : <Eye size={16} />}
                                    </Button>
                                </div>
                                <p className="text-[9px] text-muted-foreground italic">Use este campo apenas para testes rápidos ou se o fluxo OAuth falhar.</p>
                            </div>
                        </div>
            ...
            <div className="space-y-6">
                <Card className={cn("shadow-lg border-2 transition-all", isConnected ? "border-emerald-200" : "border-primary/20")}>
                    ...
                    <CardContent className="pt-6 text-center space-y-4">
                        {!isConnected ? (
                            <>
                                <p className="text-sm text-muted-foreground text-left">A autorização abre uma janela segura da Conta Azul para confirmar o acesso da OikoApp aos seus dados financeiros.</p>
