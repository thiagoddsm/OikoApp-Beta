...
export function AttendanceDashboard({ registros, loading }) {
  ...
  const dadosGraficoLinha = useMemo(() => {
    ...
    const dadosGraficoBarrasHorario = useMemo(() => {
        const totais = registrosFiltrados.reduce((acc, r) => {
            ...
            if (!acc[horario]) {
                acc[horario] = { totalAdultos: 0, countAdultos: 0, totalCriancas: 0, countCriancas: 0 };
            }
            if (r.adultos > 0) {
                acc[horario].totalAdultos += r.adultos;
                acc[horario].countAdultos += 1;
            }
            if (r.criancas > 0) {
                acc[horario].totalCriancas += (r.criancas || 0);
                acc[horario].countCriancas += 1;
            }
            return acc;
        }, {});
        ...
        return Object.entries(totais).map(([horario, data]) => ({
            name: horario,
            mediaAdultos: data.countAdultos > 0 ? Math.round(data.totalAdultos / data.countAdultos) : 0,
            mediaCriancas: data.countCriancas > 0 ? Math.round(data.totalCriancas / data.countCriancas) : 0,
   ...
   const dadosGraficoBarrasFatores = useMemo(() => {
    const calcMedia = (filtro) => {
        const { totalAdultos, countAdultos, totalCriancas, countCriancas } = registrosFiltrados.reduce((acc, r) => {
            if (filtro(r)) {
                if (r.adultos > 0) {
            ...
            return acc;
        }, { totalAdultos: 0, countAdultos: 0, totalCriancas: 0, countCriancas: 0 });
        return {
            mediaAdultos: countAdultos > 0 ? Math.round(totalAdultos / countAdultos) : 0,
            mediaCriancas: countCriancas > 0 ? Math.round(totalCriancas / countCriancas) : 0
        };
    };
  ...
  const stats = useMemo(() => {
      // Filtra registros que de fato tiveram contagem de adultos
      const registrosValidosParaMediaAdultos = registrosFiltrados.filter(r => r.adultos > 0);
      // Filtra registros que de fato tiveram contagem de crianças
      const registrosValidosParaMediaCriancas = registrosFiltrados.filter(r => r.criancas > 0);
      ...
      const mediaAdultos = registrosValidosParaMediaAdultos.length > 0
          ? Math.round(registrosValidosParaMediaAdultos.reduce((acc, r) => acc + r.adultos, 0) / registrosValidosParaMediaAdultos.length)
          : 0;
          
      const mediaCriancas = registrosValidosParaMediaCriancas.length > 0
          ? Math.round(registrosValidosParaMediaCriancas.reduce((acc, r) => acc + (r.criancas || 0), 0) / registrosValidosParaMediaCriancas.length)
          : 0;
      ...
      return {
          ...
          mediaGeral: mediaAdultos + mediaCriancas
      }
  }, [registrosFiltrados]);
