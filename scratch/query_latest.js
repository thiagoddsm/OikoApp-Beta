...
async function run() {
  console.log("Buscando últimas solicitações financeiras...");
  const snap = await db.collection('finance_requests').orderBy('createdAt', 'desc').limit(10).get();
  ...
  snap.forEach(doc => {
    ...
    console.log(`  Motivo Rejeição: ${data.rejectionReason || 'N/A'}`);
    console.log("-----------------------------------------");
  });
}
